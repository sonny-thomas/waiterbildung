...
async def get_latest_courses() -> List[Course]:
    ...
    - Returns list of the latest 9 courses sorted by creation date
    - Returns empty list if no courses found
    """
    try:
...
async def list_courses(
    ...
    try:
        courses, total = await Course.list(
            page=page, limit=limit, filters=filters, sort=sort
        )
        return CourseList(courses=courses, total=total, page=page, size=limit)
    except Exception as e:
